...
CMysqlManager::CMysqlManager(void)
{
	...
	//{
	//	STableInfo msg;
	//	msg.m_strName = "chatmsg";
	//	msg.m_mapField["ChatID"] = { "ChatID", "integer not NULL", "int(11)" };
	//	msg.m_mapField["MsgTime"] = { "MsgTime", "integer NOT NULL ", "int" };
	//	msg.m_mapField["MsgID"] = { "MsgID", "integer NOT NULL", "int(11)" };
	//	msg.m_mapField["SendID"] = { "SendID", "integer not null", "int(11)" };
	//	msg.m_mapField["Reserve"] = { "Reserve", "integer ", "" };
	//	msg.m_mapField["Msg"] = { "Msg", "blob not null", "blob" };

	//	msg.m_strKeyString = "PRIMARY KEY(MsgID,ChatID), UNIQUE KEY(MsgID,ChatID)";
	//	m_vecTableInfo.push_back(msg);
	//}

	//// 4. talkmsg 
	//{
	//	STableInfo msg;
	//	msg.m_strName = "talkmsg";
	//	msg.m_mapField["TalkID"] = { "TalkID", "BIGINT not NULL", "int(22)" };
	//	msg.m_mapField["MsgTime"] = { "MsgTime", "integer NOT NULL ", "int" };
	//	msg.m_mapField["MsgID"] = { "MsgID", "integer NOT NULL ", "int(11)" };
	//	msg.m_mapField["SendID"] = { "SendID", "integer not null", "int(11)" };
	//	msg.m_mapField["Reserve"] = { "Reserve", "integer ", "" };
	...
	//	msg.m_strKeyString = "PRIMARY KEY(MsgID,TalkID), UNIQUE KEY(MsgID,TalkID)";
	//	m_vecTableInfo.push_back(msg);
	//}
}
...
bool CMysqlManager::_CheckTable(const STableInfo& table)
{
	...
	else // 检查字段是否匹配， 暂时只检查是否存在， 还需进一步看类型是否需要修改 
	{
			...
			it != table.m_mapField.end(); ++it)
		{
			...
			if (mapOldTable.find(field.m_strName) == mapOldTable.end()) //找列名没找到，那就在数据库中插入这一行
			{
				stringstream ss;
