...
void ClientSession::OnLoginResponse(const std::string& data, const std::shared_ptr<TcpConnection>& conn)
{
    ...
    Send(outbuf);  //到这儿为止，是已知的已经过了的只有登录注册功能的部分。phase1 clear
    ...
    for (const auto& iter : friends)
    {
        ...
        if (targetSession) //如果用户在线则推送用户状态
...
void ClientSession::OnGetFriendListResponse(const std::shared_ptr<TcpConnection>& conn)
{
    ...
    for (const auto& iter : friends)
    {	
        userOnline = imserver.IsUserSessionExsit(iter.userid);  //循环遍历，判断当前用户的好友列表，判断该好友的会话是否存在，也就是它是否在线
                                                //下面的JSON，每一条就是一个好友的信息，最后有一个状态位，代表这个好友是否在线
                                                //最后将这一整个JSON全部封装，发送回去给客户端，客户端会根据最后的状态位来设置显示出来的效果
        /*
...
void ClientSession::OnFindUserResponse(const std::string& data, const std::shared_ptr<TcpConnection>& conn)
{
    ...
    string username = JsonRoot["username"].asString();
    User cachedUser;
    if (!Singleton<UserManager>::Instance().GetUserInfoByUsername(username, cachedUser))    //每次，不管在哪，使用的时候都要用单例的形式去使用
        retData = "{ \"code\": 0, \"msg\": \"ok\", \"userinfo\": [] }";                 //避免多次实例化对象浪费资源
    else
    {   //找到了该用户的信息就返回，没找到要写回去错误提示
...
{   //好友操作，也就是添加好友或者删除好友
    ...
    if (targetUserid >= GROUPID_BOUBDARY)  //比较目标用户id来分别是个人用户id还是组id(群聊id)
    {
        if (type == 4) //群聊id type=4是退群，个人id type=4是删除好友
        {
            //退群
        ...
        //加群直接同意   群聊type除了退群以外，剩下都是加群
        OnAddGroupResponse(targetUserid, conn);
        return;
    }

    //如果是个人用户id，每个type对应的类型就不一样了
    char szData[256] = { 0 };
    //删除好友
    if (type == 4)
    {
