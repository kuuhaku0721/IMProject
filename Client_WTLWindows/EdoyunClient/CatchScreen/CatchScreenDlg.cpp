...
void CCatchScreenDlg::OnPaint()
{
	...
	else
	{
		//**************************************************************************
		CPaintDC dc(this);
		TRACE("%s(%d):m_bFirstDraw %d m_bShowMsg %d\r\n",
			__FILEW__, __LINE__, m_bFirstDraw, m_bShowMsg);
		//显示截取矩形大小信息
...
//*****************************************************************************************
	...
	TRACE("%s(%d):nHitTest %d m_bFirstDraw %d\r\n",
		__FILEW__, __LINE__, nHitTest, m_bFirstDraw);
	//判断击中位置
	if (nHitTest < 0)
	{
		if (!m_bFirstDraw)
...
void CCatchScreenDlg::DrawTip()
{
	...
	string += strTemp;
	//显示到编缉框中,操作提示窗口
	m_tipEdit.SetWindowText(string + _T("<<<<<\r\n"));
	TRACE("%s(%d):", __FILEW__, __LINE__);
	TRACE(string + _T("<<<<<\r\n"));
}
...
void CCatchScreenDlg::PaintWindow()
{
	...
	InvalidateRgn(&m_rgn);//分区域刷新，是一个很好的思路
}
//改变操作提示窗口当RGB值
void CCatchScreenDlg::ChangeRGB()
{
	...
	if (strOld != string)
	{
		...
		m_tipEdit.GetWindowTextW(text);
		strSub = text.Mid(14, LineLength - 10);
		text.Replace(strSub, string);
		m_tipEdit.SetWindowTextW(text);
		TRACE("%s(%d):", __FILEW__, __LINE__);
		TRACE("LineLength %d \r\n", LineLength);
		TRACE(text + _T("\r\n"));
		TRACE(strSub + _T("\r\n"));
		m_tipEdit.Invalidate();
	}
