...
BOOL CBuddyChatDlg::HandleFile(LPCTSTR& p, tstring& strText, std::vector<CContent*>& arrContent)
{
	...
	if (!strFileName.empty())
	{
		...
		if (lpContent != NULL)
		{
			...
			lpContent->m_CFaceInfo.m_strName = Edoyun::CPath::GetFileName(strFileName.c_str());
...
void CBuddyChatDlg::OnUpdateBuddyHeadPic()
{
	...
	//if(lpBuddyInfo == NULL)
	//{
	//	::MessageBox(m_lpFMGClient->m_UserMgr.m_hCallBackWnd, _T("程序遇到一个严重的错误导致打开聊天对话框失败！"), _T("EdoyunIMClient"), MB_OK|MB_ICONERROR); 
	//	return;
	//}
...
BOOL CBuddyChatDlg::HandleFileDragResult(PCTSTR lpszFileName)
{
	...
	//如果是文件夹，则发送文件夹
	if(Edoyun::CPath::IsDirectory(lpszFileName))
	{
		//TODO: 发送文件夹
		return TRUE;
	}

	CString strFileExtension(Edoyun::CPath::GetExtension(lpszFileName).c_str());
		...
		strFileExtension==_T("gif") )
	{
		...
		{
			::MessageBox(m_hWnd, _T("图片大小超过10M，请使用截图工具。"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
			return FALSE;
		}
...
void CBuddyChatDlg::OnClose()
{
	...
	if(nFileItemCount > 0)
	{
		...
		if(IDNO == ::MessageBox(m_hWnd, _T("有文件正在传输，关闭窗口将终止传输，确实要关闭吗？"), _T("EdoyunIMClient"), MB_YESNO|MB_ICONQUESTION))
			return;
...
void CBuddyChatDlg::OnBtn_Image(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	if (fileDlg.DoModal() == IDOK)
	{
		...
		if(nFileSize > MAX_CHAT_IMAGE_SIZE)
		{
			::MessageBox(m_hWnd, _T("图片大小超过10M，请使用文件方式发送或使用截图工具。"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
			return;
		}
...
void CBuddyChatDlg::OnBtn_ScreenShot(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	{
		::MessageBox(m_hWnd, _T("启动截图工具失败！"), _T("EdoyunIMClient"), MB_OK|MB_ICONERROR);
	}
	if(pi.hProcess != NULL)
	{
...
void CBuddyChatDlg::OnBtn_Send(UINT uNotifyCode, int nID, CWindow wndCtl)
{
    if (m_lpFMGClient->IsOffline())
	{
		MessageBox(_T("您已经处于离线状态，无法发送消息，请上线后再次尝试。"), _T("EdoyunIMClient"));
		return;
	}
	...
	if (nCustomPicCnt > 1)
	{
		MessageBox(_T("每条消息最多包含1张图片，多张图片请分条发送。"), _T("EdoyunIMClient"));
		return;
	}
	...
	if (strText.empty())
	{
		::MessageBox(m_hWnd, _T("发送内容不能为空！"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
		return;
	}

	if(strText.length() > 1800)
	{
		::MessageBox(m_hWnd, _T("您发送的内容太长，请分条发送！"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
		return;
	}
...
void CBuddyChatDlg::OnMenu_SaveAs(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	if (fileDlg.DoModal() == IDOK)
	{
		...
		CString strExtName = (_T(".") + Edoyun::CPath::GetExtension(strSavePath)).c_str();
...
void CBuddyChatDlg::OnMenu_SendFile(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	{
		CString strSavePath = fileDlg.m_ofn.lpstrFile;
		CString strExtName = (Edoyun::CPath::GetExtension(strSavePath)).c_str();
		
		CString strFileTypeThumbs = Edoyun::CPath::GetAppPath().c_str();
		...
		ShowFileTransferCtrl(TRUE);
		m_staSendFileDesc.SetWindowText(Edoyun::CPath::GetFileName(strSavePath).c_str());
	}
}
...
BOOL CBuddyChatDlg::SendOfflineFile(PCTSTR pszFileName)
{
	...
	{
		::MessageBox(m_hWnd, _T("离线文件暂且不支持大小超过2G的文件。"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
		return FALSE;
	}
	...
	tstring strFileName(Edoyun::CPath::GetFileName(strSavePath));
	...
	if(!Edoyun::CPath::IsFileExist(strFileTypeThumbs))
	...
	m_FileTransferCtrl.SetItemFileSizeByID(nItemID, Edoyun::CPath::GetFileSize(pszFileName));
...
BOOL CBuddyChatDlg::RecvOfflineFile(PCTSTR lpszDownloadName, PCTSTR pszFileName, long nFileSize)
{
	...
	CString strExtName(Edoyun::CPath::GetExtension(pszFileName).c_str());
	CString strFileTypeThumbs(Edoyun::CPath::GetAppPath().c_str());
	strFileTypeThumbs += _T("\\Skins\\Skin0\\fileTypeThumbs\\");
	strFileTypeThumbs += strExtName;
	strFileTypeThumbs += _T(".png");
...
BOOL CBuddyChatDlg::OnRichEdit_LBtnDblClk(MSG* pMsg)
{
	...
	if (bRet && pImageOle != NULL)					// 双击的是图片
	{
		...
		if (-1 == nFaceId && -1 == nFaceIndex)		// 非系统表情
		{
			...
			{
				CString strExeName = Edoyun::CPath::GetAppPath().c_str();
				...
				if (Edoyun::CPath::IsFileExist(strExeName))
				{
...
LRESULT CBuddyChatDlg::OnBtn_FileTransfer(LPNMHDR pnmh)
{
	...
	CString strFileExtension(Edoyun::CPath::GetExtension(strFileName).c_str());
	if(strFileExtension == "")
		strFileExtension = _T("*");
	...
	if(pNMHDREx->btnArea == BTN_ACCEPT)
	{
		...
		if(strDefaultPath == "")
		{
			::MessageBox(m_hWnd, _T("无法保存文件至我的文档，请使用[另存为]按钮重新指定文件保存路径！"), _T("EdoyunIMClient"), MB_OK|MB_ICONERROR);
			delete pFileItemRequest;
			return 1;
		}
		CString strFilePath;
		strFilePath.Format(_T("%s\\%s"), strDefaultPath, strFileName);
		if(Edoyun::CPath::IsFileExist(strFilePath))
		{
			CString strInfo;
			strInfo.Format(_T("[%s]文件已存在，是否覆盖？"), strFilePath);
			if(IDNO == ::MessageBox(m_hWnd, strInfo, _T("EdoyunIMClient"), MB_YESNO|MB_ICONQUESTION))
			{
				delete pFileItemRequest;
				return 1;
			}
...
BOOL CBuddyChatDlg::Init()
{
	...
	if(arrSysFont.empty())
	{
		::MessageBox(m_hWnd, _T("初始化聊天对话框失败！"), _T("EdoyunIMClient"), MB_OK|MB_ICONERROR);
		return FALSE;
	}
			...
			case CONTENT_TYPE_CHAT_IMAGE:
				{
					if(nMsgType == CONTENT_TYPE_TEXT)
					{
						...
						if(!Edoyun::CPath::IsFileExist(strFileName.c_str()) || IUGetFileSize(strFileName.c_str())<=0)
						{
							m_mapRecvFileInfo.insert(std::pair<CString, long>(strFileName.c_str(), lStartChar));						
							strFileName = Edoyun::CPath::GetAppPath() + _T("Image\\DownloadImageProgress.gif");
						}
					...
					else if(nMsgType == CONTENT_TYPE_IMAGE_CONFIRM)
					{
						...
						{
							if(lpContent->m_CFaceInfo.m_strFilePath.empty() || !Edoyun::CPath::IsFileExist(strFileName.c_str()) || IUGetFileSize(strFileName.c_str())==0)
								strFileName = Edoyun::CPath::GetAppPath() + _T("Image\\DownloadFailed.gif");
...
LRESULT CBuddyChatDlg::OnSendFileProgress(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if(iter == m_mapSendFileInfo.end())
	{
		if(nPercent != -1)
		{
			...
			_stprintf_s(szProgressInfo, ARRAYSIZE(szProgressInfo), _T("正在发送图片%s：%d%%."), Edoyun::CPath::GetFileName(pFileProgress->szDestPath).c_str(), nPercent);
...
LRESULT CBuddyChatDlg::OnRecvFileResult(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if(iter->second > 0)
	{
		...
		{
			RichEdit_SetSel(m_richRecv.m_hWnd, iter->second, iter->second+1);
			tstring strFileName = Edoyun::CPath::GetAppPath() + _T("Image\\DownloadFailed.gif");
			_RichEdit_InsertFace(m_richRecv.m_hWnd, strFileName.c_str(), -1, -1);
		}
	}
	//离线文件下载结果
