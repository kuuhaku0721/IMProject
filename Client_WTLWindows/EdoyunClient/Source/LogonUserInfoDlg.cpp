...
void CLogonUserInfoDlg::OnBtn_OK(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	{
		::MessageBox(m_hWnd, _T("昵称必须在1～16个字符之间。"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
		return;
	}
	...
	{
		::MessageBox(m_hWnd, _T("个性签名必须在0～128个字符之间。"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
		return;
	}
	...
	if(st.wYear < 1900)
	{
		::MessageBox(m_hWnd, _T("生日年份必须大于1990年。"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
		return;
	}
	...
	{
		::MessageBox(m_hWnd, _T("地址长度不能超过32位。"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
		return;
	}
	...
	{
		::MessageBox(m_hWnd, _T("电话长度不能超过16位。"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
		return;
	}
	if(strPhone.GetLength()>0 && !ValidatePhone(strPhone))
	{
		::MessageBox(m_hWnd, _T("电话号码必须是数字、-和+。"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
		return;
	}
	...
	{
		::MessageBox(m_hWnd, _T("邮箱长度不能超过23位。"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
		return;
	}
...
LRESULT CLogonUserInfoDlg::OnUploadUserThumbResult(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{
		::MessageBox(m_hWnd, _T("上传头像失败！"), _T("EdoyunIMClient"), MB_OK|MB_ICONERROR);
