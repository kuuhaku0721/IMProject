...
void CSendMsgThread::HandleItem(CNetData* pNetData)
{
	...
	case NET_DATA_CHAT_MSG:
		HandleSentChatMessage((const CSendChatMessage*)pNetData);
		break;

	case NET_DATA_CHAT_CONFIRM_IMAGE_MSG:
		HandleSentConfirmImageMessage((const CSendChatConfirmImageMessage*)pNetData);
		break;
...
BOOL CSendMsgThread::HandleSentConfirmImageMessage(const CSendChatConfirmImageMessage* pConfirmImageMessage)
{
	...
	if(pConfirmImageMessage->m_nType == CHAT_CONFIRM_TYPE_SINGLE)
	{
        ...
        //消息内容
        writeStream.Write(pConfirmImageMessage->m_pszConfirmBody, nChatMsgLength);
        //targetId
        writeStream.Write((int32_t)uTargetID);
        writeStream.Flush();
...
BOOL CSendMsgThread::AddBuddyMsg(UINT nFromUin, const tstring& strFromNickName, UINT nToUin, const tstring& strToNickName, time_t nTime, const tstring& strChatMsg, HWND hwndFrom /*= NULL*/)
{
	...
	CSendChatMessage* pSentChatMessage = new CSendChatMessage();
...
BOOL CSendMsgThread::HandleCustomPic(LPCTSTR& p, tstring& strText, std::vector<CContent*>& arrContent)
{
	...
	if (!strFileName.empty())
	{
		...
		if (lpContent != NULL)
		{
			...
			lpContent->m_CFaceInfo.m_strFileName = Edoyun::CPath::GetFileName(strFileName.c_str());
...
BOOL CSendMsgThread::SendBuddyMsg(CMsgItem* lpMsgItem)
{
	...
	for (size_t i = 0; i < nSize; ++i)	
	{
		...
		if(!GetFileMd5ValueW(lpContent->m_CFaceInfo.m_strFilePath.c_str(), szMd5, ARRAYSIZE(szMd5)))
			continue;
		strDestPath.Format(_T("%s%s.%s"), m_lpUserMgr->GetChatPicFolder().c_str(), szMd5, Edoyun::CPath::GetExtension(lpContent->m_CFaceInfo.m_strFilePath.c_str()).c_str());
		...
		lpContent->m_CFaceInfo.m_strFileName = Edoyun::CPath::GetFileName(strDestPath);
	}
...
BOOL CSendMsgThread::SendMultiChatMessage(const char* pszChatMsg, int nChatMsgLength, UINT* pAccountList, int nAccountNum)
{
    std::string outbuf;
    yt::BinaryWriteStream3 writeStream(&outbuf);
    writeStream.Write(msg_type_multichat);
    writeStream.Write(m_seq);
    //senderId
    //writeStream.Write((int32_t)lpBuddyMsg->m_nFromUin);
    //消息内容
    writeStream.Write(pszChatMsg, nChatMsgLength);
    //targetId
    ...
    writeStream.Write(strTarget.c_str(), strTarget.length());
    writeStream.Flush();
...
BOOL CSendMsgThread::ProcessBuddyMsg(CBuddyMessage* lpBuddyMsg)
{
    ...
    //writeStream.Write((int32_t)lpBuddyMsg->m_nFromUin);
    //消息内容
    writeStream.Write(strUtf8Msg, nChatMsgLength);
    //targetId
    writeStream.Write((int32_t)lpBuddyMsg->m_nToUin);
    writeStream.Flush();
