...
void CRegisterDialog::OnBtn_Regist(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	{
		::MessageBox(m_hWnd, _T("无效的手机号码！"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
		m_edtRegId.SetFocus();
		return;
	}
	...
	{
		::MessageBox(m_hWnd, _T("昵称不能为空且长度不能超过15个字符！"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
	...
	{
		::MessageBox(m_hWnd, _T("密码长度必须在6～15个字符之间！"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
		return;
	}
	...
	{
		::MessageBox(m_hWnd, _T("确认密码长度必须在6～15个字符之间！"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
		return;
	}
	...
	{
		::MessageBox(m_hWnd, _T("两次输入的密码不相同，请重新确认！"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
		return;
	}
...
LRESULT CRegisterDialog::OnRegisterResult(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL &bHandled)
{
	...
	if(nRegisterResult == REGISTER_EXIST)
		::MessageBox(m_hWnd, _T("您注册的手机号已经被注册，请更换其它手机号后重试！"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
	else if(nRegisterResult == REGISTER_SUCCESS)
	{
        ...
        _stprintf_s(szSuccessInfo, 64, _T("恭喜您，注册成功，您的账号是[%s]！"), (LPCTSTR)m_strMobile);

		::MessageBox(m_hWnd, szSuccessInfo, _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);

		EndDialog(IDOK);
	}
	else
		::MessageBox(m_hWnd, _T("网络故障，注册失败，请稍后重试！"), _T("EdoyunIMClient"), MB_OK|MB_ICONERROR);
