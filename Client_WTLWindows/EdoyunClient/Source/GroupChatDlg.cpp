...
void CGroupChatDlg::OnUpdateGMemberNumber(WPARAM wParam, LPARAM lParam)
{
	...
	if (nItem != -1)
	{
		...
		if (Edoyun::CPath::IsFileExist(strFileName))
...
BOOL CGroupChatDlg::UpdateGroupMemberList()
{
	...
	for (int i = 0; i < lpGroupInfo->GetMemberCount(); i++)
	{
		...
		{
			if (!bGray)
				strFileName = (Edoyun::CPath::GetAppPath() + _T("Image\\DefGMemberHeadPic16x16.png")).c_str();
			else
				strFileName = (Edoyun::CPath::GetAppPath() + _T("Image\\DefGMemberGrayHeadPic16x16.png")).c_str();
			bGray = FALSE;
		}
