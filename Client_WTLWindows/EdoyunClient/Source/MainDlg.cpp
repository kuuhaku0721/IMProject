...
BOOL CMainDlg::OnInitDialog(CWindow wndFocus, LPARAM lInitParam)
{
	...
	if (!m_FMGClient.Init())
	{
		::MessageBox(NULL, _T("初始化失败，EdoyunIMClient无法启动！"), _T(""), MB_OK | MB_TOPMOST);
		::ExitProcess(0);
		return FALSE;
	}

	LoadAppIcon(m_FMGClient.GetStatus());
	m_TrayIcon.AddIcon(m_hWnd, WM_TRAYICON_NOTIFY, 1, m_hAppIcon, _T("EdoyunIMClient未登录"));
...
void CMainDlg::CloseDialog(int nVal)
{
	...
	{
		if (IDNO == ::MessageBox(m_hWnd, _T("您还有文件正在传输，确认要退出EdoyunIMClient？"), _T("EdoyunIMClient"), MB_YESNO | MB_ICONQUESTION))
			return;
	}
...
void CMainDlg::OnBtn_Cancel(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	{
		::MessageBox(NULL, _T("显示登录对话框失败！"), _T("EdoyunIMClient"), MB_OK | MB_ICONERROR);
	}
}
...
void CMainDlg::OnEdit_Sign_KillFocus(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	{
		::MessageBox(m_hWnd, _T("个性签名不能超过127个字符！"), _T("EdoyunIMClient"), MB_OK | MB_ICONINFORMATION);
		return;
	}
	...
	{
		::MessageBox(m_hWnd, _T("您当前处于离线状态，无法修改个性签名，请上线后重试。"), _T("EdoyunIMClient"), MB_OK | MB_ICONINFORMATION);
	}
	else
	{
		if (!strOldText.IsEmpty() && strNewText.IsEmpty())
		{
			if (::MessageBox(m_hWnd, _T("确定要更改个性签名吗？"), _T("EdoyunIMClient"), MB_YESNO | MB_ICONQUESTION) != IDYES)
			{
				bModify = FALSE;
			}
		}
...
void CMainDlg::OnBuddyListAddTeam(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	{
		::MessageBox(m_hWnd, _T("分组数量不能超过20个。"), _T("EdoyunIMClient"), MB_OK | MB_ICONINFORMATION);
		return;
	}
...
void CMainDlg::OnBuddyListDeleteTeam(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	if (m_FMGClient.m_UserMgr.m_BuddyList.GetBuddyTeamCount() <= 1)
	{
		::MessageBox(m_hWnd, _T("至少得有一个分组。"), _T("EdoyunIMClient"), MB_OK | MB_ICONINFORMATION);
		return;
	}

	if (IDYES != ::MessageBox(m_hWnd, _T("删除该分组以后，该组下的好友将移至默认分组中。\n确定要删除该分组吗？"), _T("EdoyunIMClient"), MB_YESNO | MB_ICONQUESTION))
		return;
...
void CMainDlg::OnMenu_DeleteFriend(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	{
		::MessageBox(m_hWnd, _T("请在需要删除的好友上按下右键菜单！"), _T("EdoyunIMClient"), MB_OK | MB_ICONINFORMATION);
		return;
	}
...
{
	int nRet = ::MessageBox(m_hWnd, _T("清空会话列表后无法恢复，确定要清空会话列表吗？"), _T("EdoyunIMClient"), MB_YESNO | MB_ICONWARNING);
	if (nRet != IDYES)
		return;
...
void CMainDlg::OnDeleteRecentItem(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	{
		::MessageBox(m_hWnd, _T("请在需要删除的会话上按下右键菜单！"), _T("EdoyunIMClient"), MB_OK | MB_ICONINFORMATION);
		return;
	}

	int nRet = ::MessageBox(m_hWnd, _T("确定要删除该会话吗？"), _T("EdoyunIMClient"), MB_YESNO | MB_ICONQUESTION);
	if (IDNO == nRet)
		return;
...
//#endif
	...
	{
		MessageBox(_T("网络故障，登录失败！"), _T("EdoyunIMClient"), MB_OK);
		StartLogin();
	}
	break;
	...
	{
		MessageBox(_T("用户名未注册！"), _T("EdoyunIMClient"), MB_OK);
		StartLogin();
	...
	{
		MessageBox(_T("密码错误！"), _T("EdoyunIMClient"), MB_OK);
		StartLogin();
	}
	break;
	...
	{
		ShowPanel(FALSE);		// 显示登录面板
		MessageBox(_T("服务器拒绝您的登录请求！"), _T("EdoyunIMClient"), MB_OK);
		StartLogin();
	}
...
LRESULT CMainDlg::OnBuddyMsg(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{
		tstring strFileName = Edoyun::CPath::GetAppPath() + _T("Sound\\msg.wav");	// 播放来消息提示音
...
LRESULT CMainDlg::OnSendAddFriendRequestResult(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{
		::MessageBox(m_hWnd, _T("网络故障，发送加好友请求失败，请稍后重试！"), _T("EdoyunIMClient"), MB_OK | MB_ICONERROR);
	}
...
LRESULT CMainDlg::OnRecvAddFriendRequest(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	tstring strFileName(Edoyun::CPath::GetAppPath() + _T("Sound\\system.wav"));	// 播放加好友提示音
	::sndPlaySound(strFileName.c_str(), SND_ASYNC);
...
LRESULT CMainDlg::OnDeleteFriendResult(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if (!IsGroupTarget(uAccountID))
	{
		if (wParam == DELETE_FRIEND_FAILED)
			::MessageBox(m_hWnd, _T("因网络问题，删除好友失败，请稍后重试！"), _T("EdoyunIMClient"), MB_OK | MB_ICONWARNING);
	...
	{
		::MessageBox(m_hWnd, _T("退群成功！"), _T("EdoyunIMClient"), MB_OK | MB_ICONINFORMATION);
