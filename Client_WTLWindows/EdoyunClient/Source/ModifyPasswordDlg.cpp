...
void CModifyPasswordDlg::OnBtn_OK(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	{
		::MessageBox(m_hWnd, _T("旧密码不正确！"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
	...
	{
		::MessageBox(m_hWnd, _T("新密码长度必须在3～15个字符之间！"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
	...
	{
		::MessageBox(m_hWnd, _T("两次输入的密码不相同，请确认！"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
		return;
	}

	if(strOldPassword == strNewPassword)
	{
		::MessageBox(m_hWnd, _T("新旧密码必须不相同！"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
		return;
	}
...
LRESULT CModifyPasswordDlg::OnModifyPasswordResult(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL &bHandled)
{
	...
	if(wParam == 0)
	{
		::MessageBox(m_hWnd, _T("密码修改成功，您下次可以使用新密码登录了！"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
		ShowWindow(SW_HIDE);
	}
    else if (wParam == 103)
        ::MessageBox(m_hWnd, _T("旧密码错误！"), _T("EdoyunIMClient"), MB_OK | MB_ICONINFORMATION);
	else/* if (wParam == 105)*/
		::MessageBox(m_hWnd, _T("密码修改失败，请稍后重试！"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
