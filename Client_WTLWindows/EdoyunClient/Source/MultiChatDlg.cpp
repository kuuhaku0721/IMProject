...
void CMultiChatDlg::OnBtn_Image(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	if (fileDlg.DoModal() == IDOK)
	{
		...
		{
			::MessageBox(m_hWnd, _T("图片大小超过10M，请使用截图工具或使用文件方式给对方发送。"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
			return;
		}
...
LRESULT CMultiChatDlg::OnGMemberList_DblClick(LPNMHDR pnmh)
{
	...
	if (nCurSel != -1)
	{
		...
		{
			::MessageBox(m_hWnd, _T("不能和自己聊天。"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
			return 0;
		}
		...
		if(m_lpFMGClient->m_UserMgr.IsFriend(nUTalkUin))
			...
			::MessageBox(m_hWnd, _T("暂且不支持临时会话，您必须加对方为好友以后才能与之会话。"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
	}
	return 0;
}
...
void CMultiChatDlg::OnMenu_ViewInfo(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	{
		::MessageBox(m_hWnd, _T("暂不支持查看非好友的资料。"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
...
BOOL CMultiChatDlg::Init()
{
	...
	tstring strFileName(Edoyun::CPath::GetAppPath() + _T("Image\\DefMultiChatHeadPic.png"));
