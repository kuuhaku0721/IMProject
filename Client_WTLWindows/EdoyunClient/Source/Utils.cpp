...
{
	HANDLE hMutex = ::CreateMutex(NULL, TRUE, _T("EdoyunIMClient2020"));
...
long GetFileCountOfSpecifiDirectory(PCTSTR pszDirectoryName, PCTSTR pszFileExtension)
{
	...
	while (TRUE)
	{
		if (findData.dwFileAttributes & FILE_ATTRIBUTE_NORMAL)//仅统计文件
			nCount++;
...
{
	if (!Edoyun::CPath::IsFileExist(lpSrcFile))
		return 0;
