...
void CTeamDlg::OnOK(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	{
		::MessageBox(m_hWnd, _T("分组名不能为空。"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
		return;
	}
	else if(strTeamName.GetLength() >= 16)
	{
		::MessageBox(m_hWnd, _T("分组名不能超过15个字符。"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
		return;
	}

	if(m_pFMGClient->m_UserMgr.m_BuddyList.IsTeamNameExist(strTeamName))
	{
		::MessageBox(m_hWnd, _T("分组名已经存在！"), _T("EdoyunIMClient"), MB_OK|MB_ICONINFORMATION);
		return;
	}
