	...
	if (bRet)
	{
		...
		while (bRet)
		{
			...
			if (lpFaceInfo != NULL)
			{
				...
				lpFaceInfo->m_strFileName = Edoyun::CPath::GetAppPath() + xmlSubNode.GetAttribute(_T("file"));
				tstring strIndex = Edoyun::CPath::GetFileNameWithoutExtension(lpFaceInfo->m_strFileName.c_str());
